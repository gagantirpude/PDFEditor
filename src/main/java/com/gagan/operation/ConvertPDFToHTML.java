...
public class ConvertPDFToHTML {
    ...
    public static void convertOperation(Scanner scanner) {
        ...
        System.out.print("Enter the output directory for the HTML file: ");
        ...
        try {
            ...
            try (Document pdfDocument = new Document(pdfFilePath)) {
                outputFileName = outputDirectory + File.separator + "output.html";
                pdfDocument.save(outputFileName, SaveFormat.Html);
            }
            System.out.println("PDF successfully converted to HTML: " + outputFileName);
        } catch (Exception e) {
            System.err.println("An error occurred while converting PDF to HTML: " + e.getMessage());
        }
    }
}
